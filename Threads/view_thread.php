...
// Fetch thread
$stmt = $conn->prepare("SELECT dt.*, u.F_name, u.L_name, u.User_id AS ThreadOwner 
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_SESSION['user_id'])) {
    ...
    if ($reply !== '') {
        ...
        $stmt = $conn->prepare("INSERT INTO thread_replies (Treply_content, Treply_date, Thread_id, User_id) VALUES (?, NOW(), ?, ?)");
...
$stmt = $conn->prepare("SELECT tr.*, u.F_name, u.L_name 
...
<head>
    ...
    <style>
        ...
        .fw-bold {
            color: #fff; /* White for bold text (e.g., thread title, "Replies") */
        }
        .text-muted {
            color: #ccc !important; /* Light gray for muted text (e.g., author, date) */
        }
        .btn-success {
...
<div class="card shadow-sm mb-4">
    <div class="card-body">
        ...
        <h6 class="text-muted"><i class="bi bi-person-circle"></i> <?= htmlspecialchars($thread['F_name']." ".$thread['L_name']) ?> | <i class="bi bi-tag"></i> <?= htmlspecialchars($thread['Dt_tag']) ?></h6>
        ...
        <div class="card-body d-flex justify-content-between align-items-start">
            ...
            <div class="dropdown ms-2">
                ...
                <ul class="dropdown-menu">
                    <li><a class="dropdown-item" href="edit_thread_reply.php?id=<?= $r['Reply_id'] ?>">Edit</a></li>
                    <li><a class="dropdown-item text-danger" href="delete_thread_reply.php?id=<?= $r['Reply_id'] ?>&thread=<?= $id ?>" onclick="return confirm('Delete this reply?')">Delete</a></li>
                </ul>
            </div>
            <?php endif; ?>
        </div>
